...
def load_nnunet_config(plans_path):
    ...
    # Extract number of classes from plans
    num_classes = plans.get('num_classes', None)
    if num_classes is None:
        config_2d_lookup = plans.get('configurations', {}).get('2d', {})
        num_classes = config_2d_lookup.get('num_classes', None)
    if num_classes is None:
        label_dict = plans.get('label_dict', {})
        num_classes = len(label_dict) if label_dict else None
    ...
    logging.info(f"Loaded nnUNet config: patch_size={patch_size}, norm_mean={norm_mean:.2f}, norm_std={norm_std:.2f}, num_classes={num_classes}")
...
def ScanConversionInference():
    ...
    parser.add_argument("--input-tfm-device-name", type=str, default="ProbeToReference")
    ...
    parser.add_argument("--socket-timeout", type=float, default=5.0, help="Socket timeout in seconds (default: 5.0)")
    ...
    if args.log_file:
        ...
        if log_path.is_dir():
            log_path = log_path / 'inference.log'
        logging.basicConfig(filename=str(log_path), filemode='w', level=logging.INFO)
    else:
...
def run_client(args):
    ...
    # Set socket timeout for input client
    input_client.socket.settimeout(args.socket_timeout)
    logging.info(f"Socket timeout set to {args.socket_timeout} seconds")
    logging.info(f"Connecting to PLUS input at {args.host}:{args.input_port}")
    logging.info(f"Listening for Slicer prediction output on port {args.output_port}")
    logging.info(f"Waiting for device '{args.input_device_name}' and transform '{args.input_tfm_device_name}'")
    ...
    if use_nnunet:
        ...
        # Load checkpoint first to infer output channels
        ...
        # Infer output channels from checkpoint by examining seg layer output size
        output_channels = None
        for key in state_dict.keys():
            if 'seg_layers' in key and 'weight' in key and key.endswith('.0.weight'):
                output_channels = state_dict[key].shape[0]
                logging.info(f"Inferred output_channels={output_channels} from checkpoint")
                break
        
        # Fallback to config value if not inferred
        if output_channels is None:
            output_channels = config.get('num_classes', None)
            if output_channels is None:
                output_channels = 2
            logging.info(f"Using output_channels={output_channels} from config")
        
        logging.info(f"Creating model with {output_channels} output classes...")
        ...
        try:
            ...
            logging.info("Checkpoint loaded with strict=True")
        except RuntimeError as e:
            logging.warning(f"Strict loading failed, using strict=False: {str(e)[:150]}")
            model.load_state_dict(state_dict, strict=False)
            logging.info("Checkpoint loaded with strict=False")
    ...
    logging.info(f"Input client: {args.host}:{args.input_port}")
    logging.info(f"Output server: listening on port {args.output_port}")
    logging.info(f"Waiting for messages from device: '{args.input_device_name}'")
    logging.info(f"Transform device: '{args.input_tfm_device_name}'")
    logging.info("=" * 60)
    logging.info("Waiting for OpenIGTLink messages...")
    ...
    while True:
        ...
        if image_ready and tfm_ready:
            ...
            prediction = postprocess_prediction(
                ...
                scanconversion_config=scanconversion_config,
                vertices=vertices,
                weights=weights,
...
# Postprocess prediction to output a single binary mask (background=0, foreground=255)
def postprocess_prediction(
    ...
    """
    Postprocess model prediction into a single binary mask.

    Args:
        prediction: Softmax output tensor [1, C, H, W]
        original_size: Shape of the original ultrasound frame (batch, H, W)
        scanconversion_config: Optional scan conversion config dict
        vertices, weights: Precomputed interpolation weights
        mask_array: Curvilinear mask

    Returns:
        mask_out: uint8 array [1, H, W], binary {0,255}.
    """

    # ------------------------------
    # 1. Convert to numpy and unpack
    # ------------------------------
    logits = prediction.squeeze().detach().cpu().numpy()   # [C, H, W]

    if logits.ndim != 3 or logits.shape[0] < 2:
        raise ValueError(f"Unexpected prediction shape {logits.shape}, expected at least [2, H, W]")

    # ------------------------------
    # 2. Compute argmax label map and collapse to binary mask
    # ------------------------------
    label_map = np.argmax(logits, axis=0)    # [H, W]

    # Treat any non-background class (>0) as foreground
    binary_mask = (label_map > 0).astype(np.uint8)

    # ------------------------------
    # 3. Optional scan conversion
    # ------------------------------
    if scanconversion_config is not None:
        # Convert from linear to curvilinear
        converted = scan_convert(binary_mask, scanconversion_config, vertices, weights)

        if mask_array is not None:
            converted = converted * mask_array

        mask_out = converted.astype(np.uint8)[np.newaxis, ...]

    else:
        # Resize to the original US frame size
        H, W = original_size[1], original_size[2]
        resized = cv2.resize(binary_mask, (W, H), interpolation=cv2.INTER_NEAREST)
        mask_out = resized.astype(np.uint8)[np.newaxis, ...]

    # Keep single-foreground binary semantics while matching MR-Default-friendly intensity range.
